...
- 运行：`hif-day1-change-deck-entry-20260720`。候选页右下的文档图标被误判为来源牌库入口；点击后后态为“1日目”日程记录页，不是变卡来源页。
- 已保存前后帧于 `debug/hif-live/hif-day1-change-deck-entry-20260720/`。没有选中来源卡、没有点击“チェンジ”，也没有资源消耗。
- 该临时模板和测试路由已在同轮回退；后续不得把此文档图标或其他未采证的底部图标作为牌库入口。必须先获得实际来源牌库入口的明确页面契约。

### 已验证牌库入口

- 用户确认后，以最右背包图标执行 `hif-day1-change-deck-entry-v2-20260720`；后态为 `所持スキルカード`，可见 `19` 张持有技能卡、网格和“閉じる”按钮。
- 入口模板 ROI 为 `[618, 1166, 82, 82]`，候选页锚点与模板双重命中后才允许点击；此前日程记录图标继续列为禁止入口。
- `hif-day1-change-deck-observe-v2-20260720` 已写入 Journal `20260720T235038-41008.jsonl`，仅记录当前牌库页并以 `day1_change_deck_observed_stop` 停止；未选择卡、滚动或关闭。
- 牌库完整扫描、到底判据和关闭回候选的原子快照尚未实现，不能将当前页面观察当作快照完成。
