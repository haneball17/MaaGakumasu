...
>
> **実機补注二(2026-08-20 全程実測,A——推翻上一条「stage 串联」表述)**:
> R1 后半+R2 全 12 回合逐回合実測:表盘「試験タイプ+倍率」**逐回合轮换**(R2 序列
> Vi2438/Vo1617/Da3044/Vo1617/Vi2438/Da3044/Vi2438/Da3044/Vi2438/Vo1617/Da3044/Vi2438 permil),
> 残りターン Round 级连续递减无重置(R2 12→1)、体力跨回合连续——**无多 stage 串联结构**,
> 「stage 切换」实为流行轮换。「多 stage 独立残りターン」为早期误判,roundsim 单 9/12 ターン
> +流行轮换(J3)建模与実機一致。R2 審査基準実測:三属性 1275/3064/1853(达成率 467%/879%/704%,
> 評価◎◎△)、スター性 900 達成(星 8)。優勝条件実機原文:「ラウンド１，２合計スコア1位」。
> 全程页面/UI 见 round2-settlement-ui-inventory.md。
...
> **実機补注二(2026-08-20 全程実測,A)**:培育结束(敗退流)也会生成メモリー——生成页→完了→詳細页链:
> 产物=対象偶像卡(ガラクタロード M 徽章)+スキル卡(大声援+)+**H.I.F専用アビリティ**(実機全文:「試験ごとに、以降1回まで、
> 始まりの合図使用後、スキルカード使用数追加+1・スキルカードを引く H.I.F専用重複発動不可」——即対局効果区紫/粉图标来源,
> 每メモリー一条,「試験ごとに以降1回」对应対局中「N回」计数)+**変換/再生成机制**(「あと1回可能」「初期ボーカル上昇+15」类)。
> 页面链与坐标见 round2-settlement-ui-inventory.md §8。
