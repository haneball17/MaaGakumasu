...
19. **B1 好調模板重裁升为高优先**（2026-08-23 轮 13 量化证据）：`tpl_scores` 実測 0.25-0.36（threshold 0.85）——模板大面积失效靠缓存链撑住（好調值非零但来源=缓存非现场）；需実機裁新模板（3.3.0 画面）+离线复测互配间隔；lazy 组合面板形态检测待触发实证
