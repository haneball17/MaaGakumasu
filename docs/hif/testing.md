...
`HIF Day1 读取全部换卡候选（单步）` 同样从未选中候选的 Day1 换卡候选页启动。它依次选中左、中、右槽位并把三个详情写入 Journal 的 `enumerate_target_candidates`，完成后安全停止；不会点击 `次へ`、`再抽選` 或 `チェンジ`。

2026-07-25 已在 MuMu 12 / `127.0.0.1:16416` 实测 `20260725T192206-34340`：左、中、右槽位依次读到 `頂点へ`（0.979433）、`タフネス`（0.998191）、`プライド`（0.999932）。控制器仅发送三个候选槽位点击；Journal 随后以 `target_candidate_probe_complete_stop` 安全停止，未进入 `次へ`、`再抽選` 或 `チェンジ`。

`HIF Day1 根据决策选择换卡候选（单步）` 是下一阶段的隔离入口，仍只从未选中候选的换卡候选页启动。它枚举三张候选卡，把 OCR 快照交给无设备副作用的 `decide_change_target()`，仅在每张卡都有 `skill_cards_master.json` 中的结构化元数据且评分最高者唯一时，才重新选择该候选并二次 OCR 复核；随后停在候选页。它不点击 `次へ`、不读取源牌、不重抽，也不点击 `チェンジ`。
