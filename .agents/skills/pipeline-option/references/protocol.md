...
**配套节点**：`recognition` 用 OCR 类型，`expected` 列出**所有可能的值**：

```jsonc
"SomeNode": {
    "recognition": { "type": "OCR", "param": { "expected": ["主线", "资源"], "roi": [...] } },
    "action": { "type": "Click", "param": {} }
}
```

> ⚠️ **`expected` 必须配合 OCR 类型的 `recognition`，否则不生效。** 这条反例在 SKILL.md 已说明，详见 [反模式 #4](../SKILL.md)。
