...
> **実機补注(2026-08-20,B 待复验)**:Round1 対局 UI 存在 SKIP 按钮(右下 [635,786] 附近),
> 実機单击后試験タイプ Vi→Da 直接切换、体力 +2(=examTurnEndRecoveryStamina)、手牌全换——
> SKIP 语义为「跳过整个試験(stage)」而非「空过 1 回合」;且 Round1 疑似由多个不同タイプ試験
> 串联(每 stage 独立残りターン/手牌)。若确认,roundsim 单 stage 9 ターン模型与
> GarakutaRinamiStrategy 的 SKIP 动作接线均需重审。UI 证据详见 round1-ui-inventory.md §6。
