...
- 开发、Pipeline 修改、实机测试或发现游戏机制后，必须使用项目内 `maagakumasu-documentation` Skill 做一次文档影响检查。
- 机制事实、开发决策、复盘经验和原始证据分别按 [`docs/README.md`](docs/README.md) 的边界记录；HIF 核心机制写入 [`docs/game-knowledge/hif.md`](docs/game-knowledge/hif.md)，详细页面证据与专项资料留在 `docs/hif/`；低可信观察不得驱动自动点击。
- 完成任务的代理直接写入本任务范围内的权威文档，不只提出建议或等待额外确认；仅在来源冲突、结论超出任务范围或需要外部授权时请求方向。
- HIF 资源的中文名称统一写“星星”；保留既有 `star_*` 代码标识符，不因术语统一改动接口。
- 交付时必须声明 `文档影响：`，写明更新路径或“无文档增量”。项目 Hook 只检查这一步是否完成，不自动生成知识。
